...
class Radio(object):
    ...
    def __init__(self, freqBand, nodeID, networkID=100, **kwargs):
        ...
        self.set_power_level(kwargs.get('power', 50))
        ...
        for _ in range(0, attempts):
            ...
            while (time.time() - sentTime) * 1000 < wait_time:
                if self._ACKReceived(toAddress):
                    self._debug("ack received within send")
                    return True
    ...
    def _interruptHandler(self, pin):
        self.intLock = True
        self.sendLock = False
        #ic("interrupt", self.mode == RF69_MODE_RX,
        ...
        if self.mode == RF69_MODE_RX and (irqflags2 & RF_IRQFLAGS2_PAYLOADREADY):
            ...
            if not ack_received:
                ...
                self.packets.append(Packet(int(target_id), int(sender_id), int(rssi), list(data)))
